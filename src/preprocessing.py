...
def prepare_numeric_features(df_labeled: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, pd.Series]]:
    print("=== PHASE 2: Preparing numeric features ===")
    df = df_labeled.copy()
    ...
    print(f"Numeric dtypes:\n{X_numeric.dtypes}")
    print(f"Null counts:\n{X_numeric.isnull().sum()}")
    return X_numeric, labels_dict


def handle_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    print("=== PHASE 2: Handling missing values ===")
    print(f"Null counts before:\n{df.isnull().sum()}")
    print(f"Any inf values: {np.isinf(df.values).any()}")
    ...
    if imputed_cols:
        print(f"Imputed columns: {imputed_cols}")
        print(f"Imputed counts:\n{null_counts[imputed_cols]}")
    else:
...
def normalize_features(X: pd.DataFrame, fit: bool = True, scaler: StandardScaler = None):
    if fit:
        ...
        try:
            ...
            print("[OK] Saved scaler to outputs/models/scaler.pkl")
        except OSError as exc:
            print(f"[WARNING] Failed to save scaler: {exc}")
    else:
        if scaler is None:
...
def reduce_dimensions(X_scaled: np.ndarray, n_components: int = 10):
    ...
    for idx, (r, c) in enumerate(zip(ratios, cum), start=1):
        print(f"PC{idx}: {r:.4f} (cumulative {c:.4f})")
    ...
    try:
        ...
        print(f"[OK] Saved PCA variance plot to {fig_path}")
    except OSError as exc:
        print(f"[WARNING] Failed to save PCA plot: {exc}")
    finally:
        plt.close(fig)

    try:
        joblib.dump(pca, config.MODELS_DIR / "pca_reducer.pkl")
        print("[OK] Saved PCA model to outputs/models/pca_reducer.pkl")
    except OSError as exc:
        print(f"[WARNING] Failed to save PCA model: {exc}")

    return X_reduced, pca


def create_all_splits(X_scaled: np.ndarray, labels_dict: Dict[str, pd.Series]):
    print("=== PHASE 2: Creating train/test splits ===")
    splits = {}
