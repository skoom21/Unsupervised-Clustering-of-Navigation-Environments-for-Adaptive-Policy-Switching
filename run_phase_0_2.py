...
def run_phase_0() -> None:
    print("\n" + "=" * 60)
    print("PHASE 0: Setup & Extraction")
    print("=" * 60)
...
def run_phase_1a() -> pd.DataFrame:
    print("\n" + "=" * 60)
    print("PHASE 1A: Feature Extraction")
    print("=" * 60)
...
def run_phase_1b(df_features: pd.DataFrame) -> pd.DataFrame:
    print("\n" + "=" * 60)
    print("PHASE 1B: Label Generation")
    print("=" * 60)
...
def run_phase_2(df_labeled: pd.DataFrame) -> None:
    ...
    print("[OK] Phase 2 preprocessing complete")
...
def main() -> None:
    ...
    try:
        ...
        print(f"[ERROR] Phase 1A failed: {exc}")

    if df_features is not None:
        try:
            df_labeled = run_phase_1b(df_features)
        except Exception as exc:
            print(f"[ERROR] Phase 1B failed: {exc}")

    if df_labeled is not None:
        try:
            run_phase_2(df_labeled)
        except Exception as exc:
            print(f"[ERROR] Phase 2 failed: {exc}")
